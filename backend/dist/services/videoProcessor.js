...
class VideoProcessor {
    ...
    constructor() {
        ...
        this.watermarkDir = path_1.default.join(process.cwd(), 'watermarks');
    ...
    async ensureDirectories() {
        ...
        await fs_extra_1.default.ensureDir(this.watermarkDir);
    }
    ...
    getImagePositionFilter(position) {
        const padding = 20;
        switch (position) {
            case 'top-left':
                return `x=${padding}:y=${padding}`;
    ...
    createTextFilter(settings) {
        const { text = '', fontSize = 24, color = '#ffffff', opacity = 0.8, position } = settings;
    ...
    createImageFilter(settings) {
        const { imageOpacity = 0.8, imageScale = 1.0, position } = settings;
    ...
    async processVideoWithTextWatermark(inputPath, outputPath, settings, onProgress, startTime = Date.now()) {
    ...
    async processVideoWithImageWatermark(inputPath, outputPath, settings, onProgress, startTime = Date.now()) {
        return new Promise((resolve) => {
            try {
                if (!settings.imagePath || !fs_extra_1.default.existsSync(settings.imagePath)) {
                    resolve({
                        success: false,
    ...
    async saveWatermarkImage(fileBuffer, originalName) {
        const filename = `watermark_${(0, nanoid_1.nanoid)()}_${originalName}`;
        const filePath = path_1.default.join(this.watermarkDir, filename);
        await fs_extra_1.default.writeFile(filePath, fileBuffer);
        return filePath;
    }
