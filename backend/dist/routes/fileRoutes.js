...
async function fileRoutes(fastify) {
    ...
    fastify.post('/api/process-file', async (request, reply) => {
        try {
            ...
            const fileType = fileProcessor_1.fileProcessor.getFileType(uploadedFile.mimetype);
            ...
            // Save uploaded file
            const inputPath = await fileProcessor_1.fileProcessor.saveUploadedFile(fileBuffer, uploadedFile.filename);
            // Process file with progress tracking
            const result = await fileProcessor_1.fileProcessor.processFile(inputPath, fileType, watermarkSettings, (progress) => {
                console.log(`Processing progress: ${progress}%`);
            });
            // Cleanup input file
            await fileProcessor_1.fileProcessor.cleanup(inputPath);
            if (result.success && result.outputPath) {
                // Return success with download URL
                return reply.send({
                    success: true,
                    data: {
                        processedUrl: fileProcessor_1.fileProcessor.getOutputUrl(result.outputPath),
    ...
    fastify.get('/api/file/health', async (request, reply) => {
        try {
            return reply.send({
                success: true,
