...
async function videoRoutes(fastify) {
    ...
    fastify.post('/api/upload-watermark', async (request, reply) => {
        try {
            ...
            const imagePath = await videoProcessor_1.videoProcessor.saveWatermarkImage(fileBuffer, data.filename);
            return reply.send({
                success: true,
                data: {
                    imagePath: imagePath,
    ...
    fastify.post('/api/process-video', async (request, reply) => {
        try {
            ...
            console.log('- Watermark:', watermarkSettings ? `✅ Type: ${watermarkSettings.type}` : '❌ Missing');
            // Validate video file
            if (!videoFile) {
